...
<body>
	<?php
		class Table{
			...
			public static function heatmap_all($table){
				...
				for($i=1;$i<count($table);$i++){
					for($j=1;$j<count($table[$i]);$j++){
						$Rx=(int)(($Re-$Ri)*($table[$i][$j]/$range)+$Ri);
						$Gx=(int)(($Ge-$Gi)*($table[$i][$j]/$range)+$Gi);
						$Bx=(int)(($Be-$Bi)*($table[$i][$j]/$range)+$Bi);
			...
			public static function heatmap_row($table){
				...
				for($i=1;$i<count($table);$i++){
					...
					for($j=1;$j<count($table[$i]);$j++){
						$Rx=(int)(($Re-$Ri)*($table[$i][$j]/($max-$min))+$Ri);
						$Gx=(int)(($Ge-$Gi)*($table[$i][$j]/($max-$min))+$Gi);
						$Bx=(int)(($Be-$Bi)*($table[$i][$j]/($max-$min))+$Bi);
						$color[$i][$j]="style=background-color:rgb($Rx,$Gx,$Bx);";
					}
				}
				return $color;
		...
		if(!isset($_SESSION['id'])){
			$_SESSION['id']=0;
			$_SESSION['table'][$_SESSION['id']]=
			Create_table::sales($_SESSION['id'],date("Y-m-d",strtotime(date("Y-m-d")."-1 month")),date("Y-m-d"));
			$_SESSION['id']++;
			$_SESSION['table'][$_SESSION['id']]=
			Create_table::utilizations($_SESSION['id'],date("Y-m-d",strtotime(date("Y-m-d")."-1 month")),date("Y-m-d"));
			$_SESSION['id']++;
		}
